...
bool connect(const Creds& c) {
  ...
  // 1) 임시 L2TP/IPsec 프로필 생성 (PSK). RememberCredential 끔 → 자격증명 저장 안 함.
  std::wstring add =
    L"powershell -NonInteractive -WindowStyle Hidden -Command "
    L"\"Add-VpnConnection -Name '" + std::wstring(kProfile) + L"' -ServerAddress '" + c.host + L"' "
    L"-TunnelType L2tp -L2tpPsk '" + c.psk + L"' -AuthenticationMethod MSChapv2 "
    L"-EncryptionLevel Optional -Force -RememberCredential:$false -PassThru | Out-Null\"";
